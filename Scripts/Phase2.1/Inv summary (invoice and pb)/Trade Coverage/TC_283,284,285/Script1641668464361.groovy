...
def valueDisplayedSummary = Mobile.getText(findTestObject('Phase2/BIInvoiceSummaryScreen/Value_Value'), 0)

println(valueDisplayedSummary + 'Value displayed in summary screen string format')

double ValueDisplayedSummary = Double.parseDouble(valueDisplayedSummary)

println(ValueDisplayedSummary + 'Value Displayed in Summery Screen Double Format')
...
def invAmtTxt = Mobile.getText(findTestObject('Phase2/BICollectionScreen01/InvAmt_TextView'), 0)

println(invAmtTxt + 'inv amt text view')

double InvAmt = Double.parseDouble(invAmtTxt)

println(InvAmt + 'InvAmt displayed in Collection Screen')

// TC_283 verification -- Balance Amount Lable
def ExpectedBalancetxt = findTestData('Phase2.1/TY_13/Inv summary (invoice and pb)').getValue(3, 14) + InvAmt

println(ExpectedBalancetxt + 'Expected Balance Amt lable')

Mobile.verifyElementText(findTestObject('Phase2/BICollectionScreen01/BIEfectivoScreen01/Balance_TextView'), ExpectedBalancetxt, 
    FailureHandling.STOP_ON_FAILURE)

Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Only Inv and Rep)/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC_283BalanceAmt'], 
    FailureHandling.STOP_ON_FAILURE)

// TC_283,285 verification -- Total paid Amount and Total Paid Lable
def ExpectedTotalPaidTxt = findTestData('Phase2.1/TY_13/Inv summary (invoice and pb)').getValue(3, 15) + invAmtTxt

println(ExpectedTotalPaidTxt + '---TotalpaidObjectTextView')

Mobile.verifyElementExist(findTestObject('Phase2/BICollectionScreen01/BIEfectivoScreen01/TotalPaid_TextView'), 0)

Mobile.verifyElementText(findTestObject('Phase2/BICollectionScreen01/BIEfectivoScreen01/TotalPaid_TextView'), ExpectedTotalPaidTxt, 
    FailureHandling.STOP_ON_FAILURE)

Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Only Inv and Rep)/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC_284TotalPaidAmt'], 
    FailureHandling.STOP_ON_FAILURE)

println('The Balance Amount lable and Total Piad lable are displayed as Expected')
