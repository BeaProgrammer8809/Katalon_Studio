...
def BalanceAmtInt = BalanceAmt.round()

println(BalanceAmtInt + '--Balance amt in integer format')

// 
def ExpectedBalancetxt = ((findTestData('Phase2.1/TY_13/Inv summary (invoice and pb)').getValue(3, 18) + BalanceAmt) + 
' of ') + InvAmtDouble
...
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Only Inv and Rep)/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC_283BalanceAmtLable'], 
    FailureHandling.STOP_ON_FAILURE)

println('Remaining amount is reflected in Balance Amount Lable')
