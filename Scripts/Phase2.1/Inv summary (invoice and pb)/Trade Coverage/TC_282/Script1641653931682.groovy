...
// To fetch InvoicedProducts data from Summary Screen sku pieces
...
double SalesReturnPieces = Double.parseDouble(salesReturn)

println(SalesReturnPieces + '--SalesReturn')

double AmtPerPiece = ValueDisplayedSummary / TotalPieces

println(AmtPerPiece + '--cost per piece')

double InvoicedPieces = TotalPieces - SalesReturnPieces

println(InvoicedPieces + '-- Total pieces invoiced')

double InvoicedPiecesAmt = InvoicedPieces * AmtPerPiece

println(InvoicedPiecesAmt + '--Amount to be displayed in Efectivo mode')

double SalesReturnAmt = SalesReturnPieces * AmtPerPiece

println(SalesReturnAmt + '--Amount to be displayed in CreditNote mode')
...
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Only Inv and Rep)/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC_282CreditNoteModeAmt'], 
    FailureHandling.STOP_ON_FAILURE)

Mobile.verifyElementText(findTestObject('Phase2/BICollectionScreen01/BICreditNoteScreen01/Amount_EditText'), SalesReturnAmt.toString(), 
    FailureHandling.STOP_ON_FAILURE)

Mobile.tap(findTestObject('Phase2/BICollectionScreen01/Efectivo_RadioButton'), 0)

Mobile.verifyElementText(findTestObject('Phase2/BICollectionScreen01/BIEfectivoScreen01/Amount_EditText'), InvoicedPiecesAmt.toString(), 
    FailureHandling.STOP_ON_FAILURE)

Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Only Inv and Rep)/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC_282BalanceAmt'], 
    FailureHandling.STOP_ON_FAILURE)
