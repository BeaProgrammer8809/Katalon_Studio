...
def netAmt=Double.parseDouble(invAmt1)+Double.parseDouble(invAmt2)

println('Net amount is ' + netAmt)

DecimalFormat df = new DecimalFormat('0.00')

invAmt = df.format(Double.parseDouble(invAmt))

def totalPaidAmt = Mobile.getAttribute(findTestObject('Phase2/BICollectionScreen01/BIEfectivoScreen01/Amount_EditText'),'text',0)
   
println('Total paid amount in cheque mode is ' + totalPaidAmt)

Mobile.verifyEqual(netAmt, invAmt, FailureHandling.STOP_ON_FAILURE)

println('Inv amt displayed the net amount to be paid')

'********************************************TC_ID_395**********************************************************************'

def minAmt=Mobile.getText(findTestObject('Object Repository/Phase2/BIInvoiceSummaryScreen/BICollectionScreen01/MinAmt_Value'), 0, FailureHandling.STOP_ON_FAILURE)

println "Min amount is "+minAmt

Mobile.verifyEqual(minAmt, netAmt, FailureHandling.STOP_ON_FAILURE)

println('Min amt displayed the net amount to be paid')

Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (invoice and pb)/TradeCoverage/ScreenShot'),
	[('testCaseName') : 'TC_ID_0394,TC_ID_395_CollectionScreen'], FailureHandling.STOP_ON_FAILURE)
