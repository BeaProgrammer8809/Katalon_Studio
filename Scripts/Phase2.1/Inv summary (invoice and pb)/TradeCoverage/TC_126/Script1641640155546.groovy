...
def ItemDisc00=ItemDisc.split('-')
def ItemDisc001 =ItemDisc00[1]
def ItemDiscs = ItemDisc001.replaceAll(' ', '')
println(ItemDiscs)
...
println('Since SkuGross and DiscountedSKUPrice1 are not equal item  discounts are not mapped')

Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (invoice and pb)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_126'], FailureHandling.STOP_ON_FAILURE)
