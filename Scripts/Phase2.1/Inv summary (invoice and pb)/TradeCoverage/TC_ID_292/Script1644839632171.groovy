...
// To get InvAmt Amount field
def Invamt = Mobile.getText(findTestObject('Phase2/BICollectionScreen01/InvAmt_TextView'), 0)

println(Invamt + 'InvAmt Dispalyed in collection screen')

def totalPaidText = Mobile.getText(findTestObject('Phase2/BICollectionScreen01/BIEfectivoScreen01/TotalPaid_TextView'), 
    0)

println(totalPaidText + '---Total Paid lable')

def totalPaidLen = totalPaidText.length()

def TotalPaidAmt = totalPaidText.substring(13, totalPaidLen)

println(TotalPaidAmt + '---Total Amt Paid ')

Mobile.verifyMatch(TotalPaidAmt, Invamt, false, FailureHandling.STOP_ON_FAILURE)

Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (invoice and pb)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_292TotalPaidAmt'], 
    FailureHandling.STOP_ON_FAILURE)

'To Tap on Submit button'
Mobile.tap(findTestObject('Phase2/BICollectionScreen01/Submit_Button'), 0)

// Verifying if Summary screen is Displayed after clicking on Submit Button in collection screen 
Mobile.verifyElementExist(findTestObject('Phase2/BIInvoiceSummaryScreen/Summary_ScreenTitle'), 0, FailureHandling.STOP_ON_FAILURE)

Mobile.verifyElementText(findTestObject('Phase2/BIInvoiceSummaryScreen/Summary_ScreenTitle'), findTestData('Phase2.1/TY_13/Inv summary (invoice and pb)').getValue(
        3, 20), FailureHandling.STOP_ON_FAILURE)

Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (invoice and pb)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_292AfterTappingSubmit'],
	FailureHandling.STOP_ON_FAILURE)

println("Full amount is submitted in Efectivo mode")
