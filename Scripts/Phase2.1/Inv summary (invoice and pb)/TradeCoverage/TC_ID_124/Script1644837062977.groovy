...
GlobalVariable.ProductName = findTestData('Phase2.1/CommonData/CommonData').getValue('ProductName', 21)
...
def ItemAndCategory = Mobile.getText(findTestObject('Object Repository/Phase2/BIInvoiceSummaryScreen/BIAmountSplitUpPopup01/CompDisc_Value'), 
    0)

def ItemAndCategory0=ItemAndCategory.split('-')
def ItemAndCategory011 =ItemAndCategory0[1]
def ItemAndCategoryDisc = ItemAndCategory011.replaceAll(' ', '')
println(ItemAndCategoryDisc)

Double ItemAndCategoryDiscc = ((ItemAndCategoryDisc) as Double)
...
Mobile.verifyEqual(OrderValue, OrderValue, FailureHandling.STOP_ON_FAILURE)
...
println('Since SkuGross and DiscountedSKUPrice1 are not equal ItemAndCategory discounts are mapped')

Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (invoice and pb)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_124'], FailureHandling.STOP_ON_FAILURE)
