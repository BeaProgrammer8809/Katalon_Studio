...
Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (invoice and pb)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_312'], FailureHandling.STOP_ON_FAILURE)

AppiumDriver driver = MobileDriverFactory.getDriver()

def ActualErrormessage = driver.findElementByXPath('//android.widget.Toast[1]').getText()

def ExpectedErrormessage = findTestData('Phase2.1/TY_14/TestData').getValue('ToastMessage', 28)

Mobile.verifyMatch(ActualErrormessage, ExpectedErrormessage, false, FailureHandling.STOP_ON_FAILURE)

println('Error message is matching with expected')

/*verifying collection popup is open*/
...
expectedCollectionScreenTitle = findTestData('Phase2.1/TY_14/TestData').getValue('Title', 28)
