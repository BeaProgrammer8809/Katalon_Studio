...
Mobile.setText(findTestObject('Phase2/BICollectionScreen01/BITransferenciasElectronicasScreen01/ReferenceNum_EditText'), 
    findTestData('Phase2.1/TestData/Test_Data').getValue(6, 53), 0)
...
dateresult = GenericValidator.isDate(PresentDate, "dd-MM-yyyy", true)
...
println("Since by selecting next month  button we are selecting future  date ")
...
Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (invoice and pb)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_342'],
	FailureHandling.STOP_ON_FAILURE)

def Futuredate = Mobile.getText(findTestObject('Phase2/BICollectionScreen01/BIChequesScreen01/Calendar_Button'), 0)

dateresult1 = GenericValidator.isDate(Futuredate, "dd-MM-yyyy", true)
...
Mobile.verifyNotMatch(PresentDate, Futuredate, false, FailureHandling.STOP_ON_FAILURE)
