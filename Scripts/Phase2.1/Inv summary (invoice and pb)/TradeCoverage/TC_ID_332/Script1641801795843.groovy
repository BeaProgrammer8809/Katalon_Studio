...
//float DecimalNumber= Float.parseFloat(V1)+Float.parseFloat(V2)+Float.parseFloat(V3)
//println DecimalNumber
def BeforeEnteringDecimalNumber = Mobile.getAttribute(findTestObject('Phase2/BICollectionScreen01/BITransferenciasElectronicasScreen01/ReferenceNum_EditText'), 
    'text', 0)

println(BeforeEnteringDecimalNumber)

println(BeforeEnteringDecimalNumber.length())

for (int i = 0; i < BeforeEnteringDecimalNumber.length(); i++) {
...
def AfterEnteringDecimalNumber = Mobile.getAttribute(findTestObject('Phase2/BICollectionScreen01/BITransferenciasElectronicasScreen01/ReferenceNum_EditText'),
	'text', 0)

println AfterEnteringDecimalNumber

Mobile.verifyEqual(BeforeEnteringDecimalNumber, AfterEnteringDecimalNumber, FailureHandling.STOP_ON_FAILURE)

println('Decimals is not be entering  inside the Reference No field')

Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (invoice and pb)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_ID_332'], 
