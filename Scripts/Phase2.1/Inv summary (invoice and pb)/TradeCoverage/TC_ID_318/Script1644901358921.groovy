...
def ActualFutureDate = Mobile.getText(findTestObject('Object Repository/Phase2/BICollectionScreen01/BIChequesScreen01/Calendar_Button'), 0)

LocalDate today = LocalDate.now()
LocalDate FutureMonth = today.plusMonths(1).with(TemporalAdjusters.firstDayOfMonth())
LocalDate FutureMonthwithDate1=FutureMonth.plusDays(6)
println('FutureMonthwithDate'+FutureMonthwithDate1)
String FutureDate = FutureMonthwithDate1.format(DateTimeFormatter.ofPattern("dd-MM-yyyy"));
println(FutureDate+'FutureMonthwithDate');
Mobile.verifyMatch(FutureDate, ActualFutureDate, false, FailureHandling.STOP_ON_FAILURE)

Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (invoice and pb)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_318B'],
	FailureHandling.STOP_ON_FAILURE)

Mobile.tap(findTestObject('Phase2/BICollectionScreen01/Submit_Button'), 0)

Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (invoice and pb)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_318'], 
    FailureHandling.STOP_ON_FAILURE)

def actualSummaryScreenTitle = Mobile.getText(findTestObject('Object Repository/Phase2/BIInvoiceSummaryScreen/Summary_ScreenTitle'), 
    0)

def ExpectedSummaryScreenTitle = findTestData('Phase2.1/TY_14/TestData').getValue('Title', 22)

Mobile.verifyMatch(actualSummaryScreenTitle, ExpectedSummaryScreenTitle, false, FailureHandling.STOP_ON_FAILURE)
