...
Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (invoice and pb)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_ID_419'], FailureHandling.STOP_ON_FAILURE)
...
def Balance_Amt = Mobile.getText(findTestObject('Object Repository/Phase2/BICollectionScreen01/BIEfectivoScreen01/Balance_TextView'), 
    0, FailureHandling.STOP_ON_FAILURE)

def splitTotal = Balance_Amt.split(' ')

Balance_Amt = (splitTotal[4])
...
Actual_Total_Paid_Field1 = df1.format(Float.parseFloat(Efectivo_Total_Paid))
...
println('User can able to select/switch to Transferencias mode')

//Mobile.verifyGreaterThan(Efectivo_Total_Paid, TransferenciasElectronicas_Amt, FailureHandling.STOP_ON_FAILURE)
def All_Total_Amt = Double.parseDouble(Efectivo_Amt) + Double.parseDouble(CreditNote_Amt)

Mobile.verifyEqual(Balance_Amt, All_Total_Amt, FailureHandling.STOP_ON_FAILURE)

Mobile.verifyGreaterThan(Balance_Amt, Efectivo_Amt, FailureHandling.STOP_ON_FAILURE)

println('Balance: X of XXX amount should display based on amount entered in Total paid field in Efectivo mode.')

Mobile.verifyEqual(Efectivo_Total_Paid, All_Total_Amt, FailureHandling.STOP_ON_FAILURE)

println('Total paid amount should display the amount entered in Total paid field at Efectivo mode.')
