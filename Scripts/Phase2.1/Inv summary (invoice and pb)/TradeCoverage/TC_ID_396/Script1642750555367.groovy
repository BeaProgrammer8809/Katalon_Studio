...
def expectedBalance=findTestData('Phase2/Common_Data/CommonData_03').getValue(5, 10)
...
def balanceInEffectivomode = Mobile.getText(findTestObject('Phase2/BICollectionScreen01/BIChequesScreen01/Balance_TextView'),
	0)

def splitBal=balanceInEffectivomode.split(" ")

balanceInEffectivomode=splitBal[2]

println('Balance is 0 as amount entered in total paid field of Effectivo mode')+balanceInEffectivomode

Mobile.verifyEqual(balanceInEffectivomode, expectedBalance, FailureHandling.STOP_ON_FAILURE)
...
def balanceInCreditmode = Mobile.getText(findTestObject('Phase2/BICollectionScreen01/BIChequesScreen01/Balance_TextView'),
	0)

def splitBal1=balanceInCreditmode.split(" ")

balanceInCreditmode=splitBal1[2]

println('Balance is 0 as amount entered in total paid field of Effectivo mode')+balanceInCreditmode

Mobile.verifyEqual(balanceInCreditmode, expectedBalance, FailureHandling.STOP_ON_FAILURE)
...
def splitBal2=balanceInChequemode.split(" ")

balanceInChequemode=splitBal2[2]

println('Balance is 0 as amount entered in total paid field of Effectivo mode')+balanceInChequemode

Mobile.verifyEqual(balanceInChequemode, expectedBalance, FailureHandling.STOP_ON_FAILURE)
...
def splitBal3=balanceInTransferenciasElectronicasMode.split(" ")

balanceInTransferenciasElectronicasMode=splitBal3[2]

println('Balance is 0 as amount entered in total paid field of Effectivo mode')+balanceInTransferenciasElectronicasMode

Mobile.verifyEqual(balanceInTransferenciasElectronicasMode, expectedBalance, FailureHandling.STOP_ON_FAILURE)

Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (invoice and pb)/TradeCoverage/ScreenShot'),
	[('testCaseName') : 'TC_ID_0396_CollectionScreen'], FailureHandling.STOP_ON_FAILURE)
