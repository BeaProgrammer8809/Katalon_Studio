...
Mobile.setText(findTestObject('Phase2/BICollectionScreen01/BITransferenciasElectronicasScreen01/ReferenceNum_EditText'), 
    findTestData('Phase2/TY_03/TestData_Phase2.1').getValue(2, 38), 0)
...
def len = refNum.length()

println('Length of refernece number is ' + len)

def expLen = findTestData('Phase2/Common_Data/CommonData_03').getValue(5, 31)

Mobile.verifyEqual(len, expLen, FailureHandling.STOP_ON_FAILURE)

println('Reference number is accepting 30 digit number')

Mobile.callTestCase(findTestCase('Reusable Cases/Mobile/Phase2.1/Bank_Branch'), [:], FailureHandling.STOP_ON_FAILURE)

Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (invoice and pb)/TradeCoverage/ScreenShot'), [('testCaseName') : 'TC_ID_438_CollectionScreen'], 
...
println('User is able to sumbit invoice amount with max 30 digit reference number')

Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (invoice and pb)/TradeCoverage/ScreenShot'), [('testCaseName') : 'TC_ID_438_SummaryScreen'], 
    FailureHandling.STOP_ON_FAILURE)
