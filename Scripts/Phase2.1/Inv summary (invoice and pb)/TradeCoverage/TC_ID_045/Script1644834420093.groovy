...
def CategoryDisc = Mobile.getText(findTestObject('Object Repository/Phase2/BIInvoiceSummaryScreen/BIAmountSplitUpPopup01/CompDisc_Value'), 
    0)

def CategoryDisc0=CategoryDisc.split('-')
def CategoryDiscsc =CategoryDisc0[1]
def CategoryDiscc = CategoryDiscsc.replaceAll(' ', '')
println(CategoryDiscc)

Double CategoryDiscount = ((CategoryDiscc) as Double)
...
def itemDisc = 0 //Since item disc is not mapped
...
println('Since SkuGross and DiscountedSKUPrice1 are not equal category discount is mapped')

Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (invoice and pb)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_45'], FailureHandling.STOP_ON_FAILURE)
