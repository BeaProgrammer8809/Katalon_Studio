...
def doublevalue = Double.parseDouble(InvoiceAmt11)

def ActualValue = Math.round(doublevalue)

int InvoiceAmt001 = ((ActualValue) as int)
...
def cheque=Mobile.getText(findTestObject('Phase2/BICollectionScreen01/Cheques_RadioButton'), 0)
...
def ChequAmt1 = Mobile.getText(findTestObject('Phase2/BIInvoiceSummaryScreen/BICollectionScreen01/Amount_EditText'), 0)

def doublevalue9 = Double.parseDouble(ChequAmt1)

def ActualValue9 = Math.round(doublevalue9)

int ChequAmt19 = ((ActualValue9) as int)
...
Mobile.setText(findTestObject('Phase2/BICollectionScreen01/BITransferenciasElectronicasScreen01/ReferenceNum_EditText'), 
    findTestData('Phase2.1/TestData/Test_Data').getValue(6, 55), 0)
...
def doublevalue2 = Double.parseDouble(TranfAmt1)

def ActualValue2 = Math.round(doublevalue2)

int TranfAmt = ((ActualValue2) as int)

Mobile.verifyNotMatch(InvoiceAmt11.toString(), ChequAmt19.toString(), false, FailureHandling.STOP_ON_FAILURE)
println('Half invoice amount entered in cheque mode')

Mobile.verifyEqual(CreditNoteMode, findTestData('Phase2.1/TestData/Test_Data').getValue(2, 55), FailureHandling.STOP_ON_FAILURE)
Mobile.verifyEqual(cheque, findTestData('Phase2.1/TestData/Test_Data').getValue(3, 1), FailureHandling.STOP_ON_FAILURE)
Mobile.verifyEqual(Transf, findTestData('Phase2.1/TestData/Test_Data').getValue(3, 2), FailureHandling.STOP_ON_FAILURE)

Mobile.verifyEqual(summary, findTestData('Phase2.1/TestData/Test_Data').getValue(3, 55), FailureHandling.STOP_ON_FAILURE)
println("Invoice Submitted summary screen displays")

Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (invoice and pb)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_344'], 
    FailureHandling.STOP_ON_FAILURE)
