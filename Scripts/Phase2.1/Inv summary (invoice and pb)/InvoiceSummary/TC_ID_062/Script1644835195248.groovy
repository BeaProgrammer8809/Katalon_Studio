...
Mobile.tap(findTestObject('Object Repository/Phase2/BIInvoiceSummaryScreen/I_Icon'), 0)
 ...
 def Discountitem= Mobile.getText(findTestObject('Object Repository/Phase2/BIInvoiceSummaryScreen/BIAmountSplitUpPopup01/CompDisc_Value'), 0)

 def  discountitem = Discountitem.substring(1, 5)
 
  Double DiscountItem = ((discountitem) as Double)

  def SkuGross = pieces * uprice
  
  println "Product Buying is not added to SkuGross"

def SecondGross= SkuGross - DiscountItem
def Category= 0 //since category is not mapped in this product
def Totalprice = SecondGross - Category

Mobile.verifyEqual(Buyprice, Totalprice)

Mobile.verifyEqual(Buyprice, value, FailureHandling.STOP_ON_FAILURE)
println("Since Buyprice and TotalValue are  equal so  tax  not is mapped")

Mobile.verifyNotEqual(SkuGross, Buyprice, FailureHandling.STOP_ON_FAILURE)
println("Since SkuGross and Buyprice are not equal category discount is mapped")

Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (invoice and pb)/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC_062_AmountSplitUp'], FailureHandling.STOP_ON_FAILURE)
...
Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (invoice and pb)/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC_062'], FailureHandling.STOP_ON_FAILURE)
