...
Mobile.setText(findTestObject('Phase2/BIInvoiceSummaryScreen/BIEnterTheFolioNoPopup01/FolioNo._EditText'), findTestData(
        'Phase2.1/TestData/Test_Data').getValue(8, 60), 0)
...
Mobile.setText(findTestObject('Phase2/BIInvoiceSummaryScreen/BIEnterTheFolioNoPopup01/Only_ProductBuying_InvQty/FolioProdBuy_EditText'), 
    findTestData('Phase2.1/TestData/Test_Data').getValue(9, 60), 0)

def Foliono = Mobile.getText(findTestObject('Phase2/BIInvoiceSummaryScreen/BIEnterTheFolioNoPopup01/Only_ProductBuying_InvQty/FolioProdBuy_EditText'), 
    0)

def FolioProductBuy = Mobile.getText(findTestObject('Phase2/BIInvoiceSummaryScreen/BIEnterTheFolioNoPopup01/FolioNo._EditText'), 
    0)

Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Inv, Rep and P)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_387'], 
    FailureHandling.STOP_ON_FAILURE)
...
def Expected_Print_Text = findTestData('Phase2.1/TestData/Test_Data').getValue(6, 60)


boolean FolioNum = ActualPrinttext.contains(Expected_Print_Text)

Mobile.verifyMatch(FolioNum.toString(), findTestData('Phase2.1/TestData/Test_Data').getValue(5, 59), false, FailureHandling.STOP_ON_FAILURE)

def Expected_Print_Text1 = findTestData('Phase2.1/TestData/Test_Data').getValue(7, 60)

boolean FolioPro = ActualPrinttext.contains(Expected_Print_Text1)

Mobile.verifyMatch(FolioPro.toString(), findTestData('Phase2.1/TestData/Test_Data').getValue(5, 59), false, FailureHandling.STOP_ON_FAILURE)

Mobile.verifyEqual(printPrev, findTestData('Phase2.1/TestData/Test_Data').getValue(3, 59), FailureHandling.STOP_ON_FAILURE)


Mobile.verifyEqual(FolioProductBuy, findTestData('Phase2.1/TestData/Test_Data').getValue(8, 60), FailureHandling.STOP_ON_FAILURE)
println('Folio no text filed accepting numeric characters')

Mobile.verifyEqual(Foliono, findTestData('Phase2.1/TestData/Test_Data').getValue(9, 60), FailureHandling.STOP_ON_FAILURE)
println('Folio productBuy text filed accepting numeric characters')
