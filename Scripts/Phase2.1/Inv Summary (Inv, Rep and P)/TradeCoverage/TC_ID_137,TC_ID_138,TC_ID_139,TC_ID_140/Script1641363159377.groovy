...
/*Verification done to check the we can entre the invoice amount in the TransferenciasElectronicas mode (TC_ID_137) */
...
Mobile.verifyElementText(findTestObject('Object Repository/Phase2/BICollectionScreen01/BITransferenciasElectronicasScreen01/Amount_EditText'), InvoiceAmount,FailureHandling.STOP_ON_FAILURE)
	


/*Verification done to check the Select Bank should be dropdown and It should be mandatory field (TC_ID_138) */





/*Verification done to check whether able to select Bank from dropdown (TC_ID_139)*/


Mobile.callTestCase(findTestCase('Reusable Cases/Mobile/Phase2.1/SelectBank'), [:], FailureHandling.STOP_ON_FAILURE)


/*Verification done to check Select Branch should be displayed after choosing any bank from Select Bank field (TC_ID_140)*/
...
def BranchText = Mobile.getText(findTestObject('Object Repository/Phase2/BIDropdown/Dropdown_Name'), 0)

Mobile.verifyMatch(BranchText, findTestData('Phase2.1/Common_Data/CommonData').getValue('DropdownName', 2), false,FailureHandling.STOP_ON_FAILURE)
