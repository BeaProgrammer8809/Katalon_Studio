...
discountedPrice = df.format(discountedPrice)

Mobile.verifyEqual(totalPrice, discountedPrice, FailureHandling.STOP_ON_FAILURE)

println('total price is as same as expected total price')
...
def tax = Double.parseDouble(totalPrice) * (IEPS / 100)

println('Tax included for the sku is ' + tax)

def expectedValuePrice = Double.parseDouble(discountedPrice) + (tax)
...
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Inv, Rep and P)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_ID_317_SummaryScreen'],
	FailureHandling.STOP_ON_FAILURE)
