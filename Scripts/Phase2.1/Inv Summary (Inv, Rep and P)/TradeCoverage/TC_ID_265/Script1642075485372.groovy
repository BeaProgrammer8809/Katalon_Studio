...
def productName = findTestData('Phase2.1/Common_Data/CommonData').getValue(6, 16)
...
Mobile.verifyElementText(findTestObject('Phase2/BIInvoiceSummaryScreen/BICollectionScreen01/Collection_Title'), findTestData(
        'Phase2.1/TY_06/Inv Summary (Inv, Rep and P)/TradeCoverage').getValue(2, 15), FailureHandling.STOP_ON_FAILURE)

Mobile.comment(' invoice amount should come  under bimbo legal entity')

Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Inv, Rep and P)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_ID_265'], 
...
def InvAmtText = Mobile.getText(findTestObject('Phase2/BIInvoiceSummaryScreen/BICollectionScreen01/InvAmt_Value'), 0)

double InvAmt = Double.parseDouble(InvAmtText)

InvAmt = InvAmt.round()

def LegalEntryWithAmountText = Mobile.getText(findTestObject('Phase2/BIInvoiceSummaryScreen/BICollectionScreen01/LegalEntry_TextView'), 
    0)

//LegalEntryWithAmount=Double.parseDouble(LegalEntryWithAmountText)
def LegalEntryAmountText = LegalEntryWithAmountText.substring(11)

double LegalEntryAmount = Double.parseDouble(LegalEntryAmountText)

LegalEntryAmount = LegalEntryAmount.round()

'Verify the legal entry'
Mobile.verifyMatch(findTestData('Phase2.1/TY_06/Inv Summary (Inv, Rep and P)/TradeCoverage').getValue(4, 15), LegalEntryWithAmountText.substring(
        0, 11), false, FailureHandling.STOP_ON_FAILURE)

'Verify the Invoice Amount'
Mobile.verifyEqual(InvAmt, LegalEntryAmount, FailureHandling.STOP_ON_FAILURE)
