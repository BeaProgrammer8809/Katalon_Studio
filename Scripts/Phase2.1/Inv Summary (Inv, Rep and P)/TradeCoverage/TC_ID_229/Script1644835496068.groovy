...
println(InvoiceAmount.length())

for (int i = 0; i < (InvoiceAmount.length() + 1); i++) {
...
Mobile.longPress(findTestObject('Phase2/BICollectionScreen01/BITransferenciasElectronicasScreen01/Amount_EditText'), 0)
...
EnteredInvAmtInTransferenciasElectronicas = Mobile.getText(findTestObject('Phase2/BICollectionScreen01/BITransferenciasElectronicasScreen01/Amount_EditText'), 
    0)

Mobile.verifyMatch(InvoiceAmount, EnteredInvAmtInTransferenciasElectronicas, false, FailureHandling.STOP_ON_FAILURE)

Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Inv, Rep and P)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_229'], 
    FailureHandling.STOP_ON_FAILURE)
