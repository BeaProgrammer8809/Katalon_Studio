...
*/
def Invamount =Double.parseDouble(InvoiceAmountInSummary)

def InVAmount = Math.round(Invamount)

int InvoiceAomuntInSummary = (int)InVAmount

Mobile.tap(findTestObject('Object Repository/Phase2/BIInvoiceSummaryScreen/Collection_Icon'), 0)

/*Verification done to check Total Paid:Invoice amount is displaying  (TC_ID_96)*/

def ActualTotalPaidText=Mobile.getText(findTestObject('Object Repository/Phase2/BICollectionScreen01/BIEfectivoScreen01/TotalPaid_TextView'), 0)

def TotalPaid=findTestData('Phase2.1/TY_05/Testdata').getValue('Data1', 14)

def ExpectedTotalPaidText= TotalPaid + ' ' + Invamount.toString()

Mobile.verifyMatch(ActualTotalPaidText, ExpectedTotalPaidText, false,FailureHandling.STOP_ON_FAILURE)

Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Inv, Rep and P)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_ID_097'],
