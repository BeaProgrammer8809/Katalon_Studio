...
def InvoiceAmountInSummary = Mobile.getText(findTestObject('Object Repository/Phase2/BIInvoiceSummaryScreen/Value_Value'),
	0)
def InvoiceValueAmount=Double.parseDouble(InvoiceAmountInSummary)

def halfAmount = (InvoiceValueAmount/2)
...
/*Verification done to check the We can able to submit half half amount in both RTGS and Cheques mode means(if we can able to submit then the collection popup will be hidden and the Preticket icon will be hidden in the summary screen)*/
...
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Inv, Rep and P)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_ID_155'],
