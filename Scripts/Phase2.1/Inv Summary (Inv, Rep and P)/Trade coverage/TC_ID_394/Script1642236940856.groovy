...
Mobile.setText(findTestObject('Phase2/BIInvoiceSummaryScreen/BIEnterTheFolioNoPopup01/FolioNo._EditText'), findTestData(
		'Phase2.1/TY_13/Inv Summary (Inv, Rep and P').getValue(3, 7), 0)

Mobile.setText(findTestObject('Phase2/BIInvoiceSummaryScreen/BIEnterTheFolioNoPopup01/FolioProdBuy_EditText'), findTestData(
		'Phase2.1/TY_13/Inv Summary (Inv, Rep and P').getValue(3, 7), 0)

def ExpectedText = findTestData('Phase2.1/TY_13/Inv Summary (Inv, Rep and P').getValue(3, 7)
println(ExpectedText + '--ActualText to enter in Folio No')
def LenOfExpectedtext = ExpectedText.length()
println(LenOfExpectedtext + '--Length of the text that is supposed to be enterd')

def ActualFolioNo = Mobile.getText(findTestObject('Phase2/BIInvoiceSummaryScreen/BIEnterTheFolioNoPopup01/FolioNo._EditText'),
	0)
def LenActualFolioNo = ActualFolioNo.length()
...
def ActualFolioProdBuy = Mobile.getText(findTestObject('Phase2/BIInvoiceSummaryScreen/BIEnterTheFolioNoPopup01/FolioProdBuy_EditText'),
	0)
def LenActualFolioProdBuy = ActualFolioProdBuy.length()

//def ExpectedText = findTestData('Phase2.1/TY_13/Inv Summary (Inv, Rep and P').getValue(3, 7)

Mobile.verifyNotMatch(ActualFolioNo, ExpectedText, false, FailureHandling.STOP_ON_FAILURE)

Mobile.verifyNotMatch(ActualFolioProdBuy, ExpectedText, false, FailureHandling.STOP_ON_FAILURE)

Mobile.verifyNotEqual(ActualFolioNo, LenOfExpectedtext)
Mobile.verifyNotEqual(ActualFolioProdBuy, LenOfExpectedtext)

Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Inv, Rep and P)/Trade coverage/Screenshot'), [('testCaseName') : 'TC_ID_394EnteredFolioInput'],
	FailureHandling.STOP_ON_FAILURE)
...
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Inv, Rep and P)/Trade coverage/Screenshot'), [('testCaseName') : 'TC_ID_394PrintPreviewScreen'],
	FailureHandling.STOP_ON_FAILURE)

println("FolioNo cannot be More than 15 cahracters, even if tried to enter more than 15 only first 15 chars is accepted.")
