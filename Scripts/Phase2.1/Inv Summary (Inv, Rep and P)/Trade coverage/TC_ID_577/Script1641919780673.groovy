...
Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (invoice and pb)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_ID_577'],
	FailureHandling.STOP_ON_FAILURE)
...
def Expected_Print_Text = findTestData('Phase2.1/TY_12/Inv Summary (Inv, Rep and P)/TestData').getValue(4, 30)

println(Expected_Print_Text + 'Expected_Print_Text')

boolean RFC = ActualPrinttext.contains(Expected_Print_Text)

Mobile.verifyMatch(RFC.toString(), findTestData('Phase2.1/TY_12/Inv Summary (Inv, Rep and P)/TestData').getValue(5, 30), 
