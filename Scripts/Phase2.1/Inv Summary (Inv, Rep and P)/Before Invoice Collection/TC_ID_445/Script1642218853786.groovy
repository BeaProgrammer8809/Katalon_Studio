...
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Inv, Rep and P)/Before Invoice Collection/Screenshot'), [('testCaseName') : 'TC_ID_445_Actual_Pieces_Value'], FailureHandling.STOP_ON_FAILURE)
...
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Inv, Rep and P)/Before Invoice Collection/Screenshot'), [('testCaseName') : 'TC_ID_445_Expected_Values'], FailureHandling.STOP_ON_FAILURE)
...
def Amount=Mobile.getText(findTestObject('Phase2/BICollectionScreen01/BIEfectivoScreen01/Amount_EditText'), 0)
DecimalFormat df = new DecimalFormat('0.00')
def Actual_Amount=df.format(Double.parseDouble(Amount))

//Verification to check that the invoice amount is not cleared in Efectivo mode
Mobile.verifyEqual(Actual_Inv_Amount, Actual_Amount, FailureHandling.STOP_ON_FAILURE)
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Inv, Rep and P)/Before Invoice Collection/Screenshot'), [('testCaseName') : 'TC_ID_445_Invoice_Amount_Not_Cleared'], FailureHandling.STOP_ON_FAILURE)
...
//Verification to check that $3 is not displayed in Efectivo mode
...
def Expected_Dollar_Amt=findTestData('Phase2.1/TY_02/Test_Data_Phase2').getValue(4, 70)

Mobile.verifyNotEqual(Actual_Dollar_Amt, Expected_Dollar_Amt, FailureHandling.STOP_ON_FAILURE)
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Inv, Rep and P)/Before Invoice Collection/Screenshot'), [('testCaseName') : 'TC_ID_445_$3_Is_Not_Displayed_In_Efectivo_Mode'], FailureHandling.STOP_ON_FAILURE)
