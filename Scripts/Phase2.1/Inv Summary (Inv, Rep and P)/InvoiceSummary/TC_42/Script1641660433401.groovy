...
def ProductQty = Mobile.getText(findTestObject('Phase2/BIReturnProductBuyingScreen01/Pieces_Edit_Text_Indexing'), 0)
...
def ItemDisc = Mobile.getText(findTestObject('Object Repository/Phase2/BIInvoiceSummaryScreen/BIAmountSplitUpPopup01/CompDisc_Value'), 
    0)

//def ItemDisc=ItemDisc.length()
def ItemDisc0=ItemDisc.split('-')
def ItemDisc0s =ItemDisc0[1]
def ItemDiscs = ItemDisc0s.replaceAll(' ', '')


Double ItemDiscss = ((ItemDiscs) as Double)
println(ItemDiscss)
...
def Category = 0 //Since Category disc is not mapped

def SecondGross = SkuGross - ItemDiscss
...
//Mobile.verifyEqual(DiscountedSKUPrice1, Totalprice, FailureHandling.STOP_ON_FAILURE)
...
println('Since SkuGross and DiscountedSKUPrice1 are not equal Item discount is mapped')

Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Inv, Rep and P)/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC_42'], FailureHandling.STOP_ON_FAILURE)
