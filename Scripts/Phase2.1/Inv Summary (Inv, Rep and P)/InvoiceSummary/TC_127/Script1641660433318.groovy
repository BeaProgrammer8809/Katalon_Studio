...
def catDisc = Mobile.getText(findTestObject('Object Repository/Phase2/BIInvoiceSummaryScreen/BIAmountSplitUpPopup01/CompDisc_Value'), 
    0)

def Category9 = catDisc.split('-')

def Category90 = Category9[1]

def catDiscs = Category90.replaceAll(' ', '')

println(catDiscs)

Double catDiscount = ((catDiscs) as Double)
...
println('Since SkuGross and DiscountedSKUPrice1 are not equal Category  discounts are not mapped')

Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Inv, Rep and P)/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC_127'], FailureHandling.STOP_ON_FAILURE)
