...
Mobile.setText(findTestObject('Object Repository/Phase2/BICollectionScreen01/BIChequesScreen01/ChequeNum_EditText'), findTestData(
        'Phase2.1/TY_06/Invoice, Replacement with Sp/invoice-Delivery Summary-Delivery Final Summary').getValue(3, 21), 
    0)

Mobile.callTestCase(findTestCase('Phase2.1/InvoiceReplacementWithSp/InvoiceDeliverySummaryDeliveryFinalSummary/Screenshot'), 
    [('testCaseName') : 'TC_ID_281'], FailureHandling.STOP_ON_FAILURE)

def expectedchequeNo = findTestData('Phase2.1/TY_06/Invoice, Replacement with Sp/invoice-Delivery Summary-Delivery Final Summary').getValue(
    3, 21)

def actualChequeNO = Mobile.getText(findTestObject('Object Repository/Phase2/BICollectionScreen01/BIChequesScreen01/ChequeNum_EditText'), 
    0)

int length = actualChequeNO.length()

Mobile.comment('Verify the ChequeNo takes max digit')

Mobile.verifyMatch(expectedchequeNo, actualChequeNO, false, FailureHandling.STOP_ON_FAILURE)

Mobile.comment('Verify the max digit was 20')

Mobile.verifyEqual(length, 20, FailureHandling.STOP_ON_FAILURE)
