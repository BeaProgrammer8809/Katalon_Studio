...
GlobalVariable.ProductName = findTestData('Phase2.1/Common_Data/CommonData').getValue('ProductName', 60)
...
GlobalVariable.ProductName = findTestData('Phase2.1/Common_Data/CommonData').getValue('ProductName', 50)
...
((driver) as AndroidDriver<MobileElement>).pressKey(new KeyEvent(AndroidKey.A))
...
GlobalVariable.index = findTestData('Phase2.1/Common_Data/CommonData').getValue('Number', 3)
def FolioNum1= Mobile.getText(findTestObject('Phase2/BIDeliveryFinalSummaryScreen/Folio_NO_Indexing'), 0)

def AcceptFolioNum1= findTestData('Phase2.1/TY_05/Collection').getValue(3, 77)

Mobile.verifyNotMatch(FolioNum1, AcceptFolioNum1, false, FailureHandling.STOP_ON_FAILURE)
...
def Expectedtoastmessage = findTestData('Phase2.1/TY_05/Collection').getValue(3, 78)
...
println "It should be mandatory to enter against each invoice name"
Mobile.callTestCase(findTestCase('Phase2.1/InvoiceSplitOnlyPB/ProductBuyingDeliveryFinalSummary/Screenshot'), [('testCaseName') : 'TC_87'], FailureHandling.STOP_ON_FAILURE)
