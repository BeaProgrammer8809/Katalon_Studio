...
GlobalVariable.ProductName = findTestData('Phase2/Common_Data/CommonData_03').getValue(6, 33)
...
def secondGrossAmt = Double.parseDouble(totalValue)
...
def actualDiscount = Double.parseDouble(totalValue) - Double.parseDouble(amount1)

actualDiscount=df.format(actualDiscount)

Mobile.verifyEqual(actualDiscount, catDisc.toString(), FailureHandling.STOP_ON_FAILURE)

println "Category discount is applied for sku"

def totalSum = secondGrossAmt - catDisc

def tax = Double.parseDouble(findTestData('Phase2/Common_Data/CommonData_03').getValue(5, 10))

def expAmount = totalSum + tax
...
Mobile.callTestCase(findTestCase('Phase2.1/InvoiceSplitOnlyPB/ProductBuyingDeliveryFinalSummary/Screenshot'), ['testCaseName':'TC_69_DeliveryFinalSummaryScreen'], FailureHandling.STOP_ON_FAILURE)
