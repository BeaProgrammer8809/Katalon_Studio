...
GlobalVariable.ProductName = findTestData('Phase2/Common_Data/CommonData_03').getValue(6, 30)
...
def totalSum = Double.parseDouble(totalValue)

def billDisc = CustomKeywords.'com.ty.keywords.MobileKeywords.billDiscount'(totalSum)
...
Mobile.verifyEqual(billDisc, discount.toString(), FailureHandling.STOP_ON_FAILURE)

println('Bill discount is applied for sku')

def totalPrice=totalSum-billDisc

def tax = Double.parseDouble(findTestData('Phase2/Common_Data/CommonData_03').getValue(5, 10))

def expAmount = totalPrice + tax
...
Mobile.callTestCase(findTestCase('Phase2.1/InvoiceSplitOnlyPB/ProductBuyingDeliveryFinalSummary/Screenshot'), [('testCaseName') : 'TC_70_DeliveryFinalSummaryScreen'], 
