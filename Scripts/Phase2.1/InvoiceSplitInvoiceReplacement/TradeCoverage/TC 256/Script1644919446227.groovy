...
Mobile.callTestCase(findTestCase('Phase2.1/InvoiceSplitInvoiceReplacement/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_256EnteredFolioNo'],
...
Mobile.callTestCase(findTestCase('Phase2.1/InvoiceSplitInvoiceReplacement/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_256ToastMsg'],
	FailureHandling.STOP_ON_FAILURE)

def Actual_ToastMessage = driver.findElementByXPath('//android.widget.Toast[1]').getText()

def Expected_ToastMessage = findTestData('Phase2.1/TY_13/SplitInvoiceReplacementAndProductBuying').getValue(3, 11)

Mobile.verifyMatch(Actual_ToastMessage, Expected_ToastMessage, false, FailureHandling.STOP_ON_FAILURE)

println("We cannot enter less than 6 cahracters for Folio No field")
