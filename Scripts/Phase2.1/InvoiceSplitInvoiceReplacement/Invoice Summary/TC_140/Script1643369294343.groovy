...
Mobile.callTestCase(findTestCase('Reusable Cases/Mobile/Phase2.1/Invoice Qty'), [('testData1') : findTestData('Phase2.1/Common_Data/CommonData').getValue(
            16, 22)], FailureHandling.STOP_ON_FAILURE)

Mobile.callTestCase(findTestCase('Reusable Cases/Mobile/Phase2.1/Invoice Qty'), [('testData1') : findTestData('Phase2.1/Common_Data/CommonData').getValue(
	16, 11)], FailureHandling.STOP_ON_FAILURE)
...
def Actual_Invoice_Qty1 = Mobile.getText(findTestObject('Object Repository/Phase2/BIInvoiceSummaryScreen/BISummaryProductDetails/Pieces_Value_Indexing'), 
    0, FailureHandling.STOP_ON_FAILURE)
...
def Actual_Invoice_Qty2 = Mobile.getText(findTestObject('Object Repository/Phase2/BIInvoiceSummaryScreen/BISummaryProductDetails/Pieces_Value_Indexing'),
	0, FailureHandling.STOP_ON_FAILURE)


def Actual_Invoice_Qty = Integer.parseInt(Actual_Invoice_Qty1) + Integer.parseInt(Actual_Invoice_Qty2)
...
Mobile.callTestCase(findTestCase('Phase2.1/InvoiceSplitInvoiceReplacement/Invoice Summary/Screenshot'), [('testCaseName') : 'TC_140'], 
    FailureHandling.STOP_ON_FAILURE)
...
def Expected_Invoice_Popup_Text = findTestData('Phase2.1/TY_04/Phase2.1_Sheet3').getValue(16, 109)

boolean Invoice_PopUp_Text = Actual_Invoice_And_UUID_PopUp_Text.contains(Expected_Invoice_Popup_Text)

def Expected_UUID_Popup_Text = findTestData('Phase2.1/TY_04/Phase2.1_Sheet3').getValue(17, 109)
...
Mobile.verifyMatch(Invoice_PopUp_Text.toString(), findTestData('Phase2.1/TY_04/Phase2.1_Sheet3').getValue(18, 109), false, 
    FailureHandling.STOP_ON_FAILURE)

//Verification to check that UUID is gerenated 
Mobile.verifyMatch(UUID_PopUp_Text.toString(), findTestData('Phase2.1/TY_04/Phase2.1_Sheet3').getValue(18, 109), false, 
    FailureHandling.STOP_ON_FAILURE)
...
Mobile.verifyMatch(Expected_Invoice_Qty, Actual_Invoice_Qty.toString(), false, FailureHandling.STOP_ON_FAILURE)
