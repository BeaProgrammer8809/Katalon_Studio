...
Mobile.verifyMatch(verifytheMessage1, findTestData('Phase2.1/TY_13/SplitInvoiceReplacementAndProductBuying').getValue(
		5, 40), false, FailureHandling.STOP_ON_FAILURE)
...
def onsitExpectedMessage = findTestData('Phase2.1/TY_13/SplitInvoiceReplacementAndProductBuying').getValue('Data3', 40)

boolean OnsitMessageResult = actualMessage2.contains(onsitExpectedMessage)

def verifytheStatus = OnsitMessageResult.toString()

Mobile.verifyMatch(verifytheStatus, findTestData('Phase2.1/TY_13/SplitInvoiceReplacementAndProductBuying').getValue('Expected Boolian value 2', 40), false, FailureHandling.STOP_ON_FAILURE)
