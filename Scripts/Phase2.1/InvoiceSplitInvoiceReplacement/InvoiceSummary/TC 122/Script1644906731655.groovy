...
Mobile.callTestCase(findTestCase('Reusable Cases/Mobile/Phase2.1/OnlyProduct_Buying_Quantity'), [('testData1') : findTestData(
			'Phase2.1/Common_Data/CommonData').getValue('Onsite Products', 3)], FailureHandling.STOP_ON_FAILURE)
...
Mobile.callTestCase(findTestCase('Phase2.1/InvoiceSplitInvoiceReplacement/InvoiceSummary/Screenshot'),
	[('testCaseName') : 'TC_122DeliveryFinalSummary'], FailureHandling.STOP_ON_FAILURE)
...
Mobile.callTestCase(findTestCase('Phase2.1/InvoiceSplitInvoiceReplacement/InvoiceSummary/Screenshot'),
	[('testCaseName') : 'TC_122SplittedInvoice-UUID'], FailureHandling.STOP_ON_FAILURE)
...
def ExpectedResult = findTestData('Phase2.1/TY_13/SplitInvoiceReplacementAndProductBuying').getValue(3, 36)

def ExpectedResult1 = findTestData('Phase2.1/TY_13/SplitInvoiceReplacementAndProductBuying').getValue(7, 36)
...
Mobile.verifyMatch(verifytheMessage, findTestData('Phase2.1/TY_13/SplitInvoiceReplacementAndProductBuying').getValue(6,
		36), false, FailureHandling.STOP_ON_FAILURE)
...
Mobile.verifyMatch(verifytheMessage1, findTestData('Phase2.1/TY_13/SplitInvoiceReplacementAndProductBuying').getValue(
		5, 36), false, FailureHandling.STOP_ON_FAILURE)
...
def onsitExpectedMessage = findTestData('Phase2.1/TY_13/SplitInvoiceReplacementAndProductBuying').getValue('Data3', 36)

boolean OnsitMessageResult = actualMessage2.contains(onsitExpectedMessage)

def verifytheStatus = OnsitMessageResult.toString()

Mobile.verifyMatch(verifytheStatus, findTestData('Phase2.1/TY_13/SplitInvoiceReplacementAndProductBuying').getValue('Expected Boolian value 2', 36), false, FailureHandling.STOP_ON_FAILURE)
