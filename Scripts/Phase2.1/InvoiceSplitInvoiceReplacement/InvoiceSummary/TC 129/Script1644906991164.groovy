...
Mobile.callTestCase(findTestCase('Reusable Cases/Mobile/Phase2.1/OnlyProduct_Buying_Quantity'), [('testData1') : findTestData(
			'Phase2.1/Common_Data/CommonData').getValue('Onsite Products', 24)], FailureHandling.STOP_ON_FAILURE)
...
Mobile.callTestCase(findTestCase('Phase2.1/InvoiceSplitInvoiceReplacement/InvoiceSummary/Screenshot'),
	[('testCaseName') : 'TC_129DeliveryFinalSummary'], FailureHandling.STOP_ON_FAILURE)
...
Mobile.callTestCase(findTestCase('Phase2.1/InvoiceSplitInvoiceReplacement/InvoiceSummary/Screenshot'),
	[('testCaseName') : 'TC_129SplittedInvoice-UUID'], FailureHandling.STOP_ON_FAILURE)
...
def ExpectedResult = findTestData('Phase2.1/TY_13/SplitInvoiceReplacementAndProductBuying').getValue(3, 43)

def ExpectedResult1 = findTestData('Phase2.1/TY_13/SplitInvoiceReplacementAndProductBuying').getValue(7, 43)
...
Mobile.verifyMatch(verifytheMessage, findTestData('Phase2.1/TY_13/SplitInvoiceReplacementAndProductBuying').getValue(6,
		43), false, FailureHandling.STOP_ON_FAILURE)
...
Mobile.verifyMatch(verifytheMessage1, findTestData('Phase2.1/TY_13/SplitInvoiceReplacementAndProductBuying').getValue(
		5, 43), false, FailureHandling.STOP_ON_FAILURE)
...
def onsitExpectedMessage = findTestData('Phase2.1/TY_13/SplitInvoiceReplacementAndProductBuying').getValue('Data3', 43)

boolean OnsitMessageResult = actualMessage2.contains(onsitExpectedMessage)

def verifytheStatus = OnsitMessageResult.toString()

Mobile.verifyMatch(verifytheStatus, findTestData('Phase2.1/TY_13/SplitInvoiceReplacementAndProductBuying').getValue('Expected Boolian value 2', 43), false, FailureHandling.STOP_ON_FAILURE)
