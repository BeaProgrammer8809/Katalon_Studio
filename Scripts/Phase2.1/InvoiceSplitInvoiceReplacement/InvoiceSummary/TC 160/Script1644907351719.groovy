...
Mobile.callTestCase(findTestCase('Reusable Cases/Mobile/Phase2.1/Product_Buying_Quantity'), [('testData1') : findTestData(
            'Phase2.1/Common_Data/CommonData').getValue('Onsite Products', 14)], FailureHandling.STOP_ON_FAILURE)
...
Mobile.setText(findTestObject('Phase2/BIDeliveryFinalSummaryScreen/Folio_NO_Indexing'), findTestData('Phase2.1/Common_Data/CommonData').getValue(
        'Folio number', 1), 0)
...
def UUIDMsg = Mobile.getText(findTestObject('Phase2/BIDeliveryFinalSummaryScreen/Splitted_Invoice_Saved_Successfully._TextView'), 
    0)

println(UUIDMsg)

def expectedMessage = findTestData('Phase2.1/TY_01/TestData3').getValue(7, 19)

boolean UUIDmsg = UUIDMsg.contains(expectedMessage)

def verifyMessage = UUIDmsg.toString()

Mobile.verifyMatch(verifyMessage, findTestData('Phase2.1/TY_01/TestData3').getValue(8, 19), false, FailureHandling.STOP_ON_FAILURE)

println "UUID is not Generated"
...
def expectedMessage1 = findTestData('Phase2.1/TY_01/TestData3').getValue(4, 19)

boolean MessageResult = actualMessage.contains(expectedMessage1)

def verifytheToastMessage = MessageResult.toString()

Mobile.verifyMatch(verifytheToastMessage, findTestData('Phase2.1/TY_01/TestData3').getValue(6, 19), false, FailureHandling.STOP_ON_FAILURE)

Mobile.callTestCase(findTestCase('Phase2.1/InvoiceSplitInvoiceReplacement/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC_160'], 
