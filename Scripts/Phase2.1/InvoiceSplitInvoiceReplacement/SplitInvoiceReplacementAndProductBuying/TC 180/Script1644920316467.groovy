...
int EffectioCashAmount = ((Cash_Amount) as int)

println(EffectioCashAmount)
...
def Actual_Amt=(inv_amt) - (Credit_Amt)
println Actual_Amt

Actual_Amount = Math.round(Actual_Amt)

def ActualAmount=(int)Actual_Amount

//Mobile.verifyMatch(ActualAmount, Cash_Amt, false, FailureHandling.STOP_ON_FAILURE)

Mobile.verifyEqual(ActualAmount.toString(), EffectioCashAmount.toString(), FailureHandling.STOP_ON_FAILURE)

println "Product buying amount is  adjusted in invoice amount "
...
Mobile.callTestCase(findTestCase('Phase2.1/InvoiceSplitInvoiceReplacement/SplitInvoiceReplacementAndProductBuying/Screenshot'),
	[('testCaseName') : 'TC_180'], FailureHandling.STOP_ON_FAILURE)
