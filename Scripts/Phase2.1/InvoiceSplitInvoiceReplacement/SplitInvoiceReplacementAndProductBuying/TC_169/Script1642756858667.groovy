...
def ExtInvAmnt = Mobile.getText(findTestObject('Phase2/BIDeliveryFinalSummaryScreen/Amount_Value_Indexing'), 0)

println(ExtInvAmnt)

def ExtInvoiceAmt = Double.parseDouble(ExtInvAmnt)

def ExtraInvoiceAmount = Math.round(ExtInvoiceAmt)

int ActualExtraInvoiceAmount = ((ExtraInvoiceAmount) as int)

println(ActualExtraInvoiceAmount)
...
int ExpectedInvoiceAmount = ((InvoiceAmount) as int)

println(ExpectedInvoiceAmount)

def legalEntity = Mobile.getText(findTestObject('Phase2/BIInvoiceSummaryScreen/BICollectionScreen01/LegalEntry_TextView'), 
    0)

println(legalEntity)

def split = legalEntity.split(':')

Actual_legalEntity = (split[0])

println(Actual_legalEntity)

Mobile.verifyMatch(Actual_legalEntity, findTestData('Phase2.1/TY_01/TestData2').getValue(4, 65), false, FailureHandling.STOP_ON_FAILURE)

println(' Legal entity is verified')

Mobile.verifyMatch(ActualExtraInvoiceAmount.toString(), ExpectedInvoiceAmount.toString(), false, FailureHandling.STOP_ON_FAILURE)

println('extraInvoice is verifed')
...
println " Extra Invoice Amount is collected"
...
Mobile.callTestCase(findTestCase('Phase2.1/InvoiceSplitInvoiceReplacement/SplitInvoiceReplacementAndProductBuying/Screenshot'),
	[('testCaseName') : 'TC_169'], FailureHandling.STOP_ON_FAILURE)
