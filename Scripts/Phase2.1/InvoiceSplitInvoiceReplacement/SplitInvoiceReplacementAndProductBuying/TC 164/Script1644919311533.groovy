...
def InvoiceSplit = Mobile.getText(findTestObject('Phase2/BIDeliveryFinalSummaryScreen/Invoice Split_Indexing'), 0)

println(InvoiceSplit)
...
def InvoiceSplitAmount=Mobile.getText(findTestObject('Phase2/BIDeliveryFinalSummaryScreen/Amount_Value_Indexing'), 0)

println InvoiceSplitAmount

Mobile.verifyEqual(TotalValue, InvoiceSplitAmount, FailureHandling.STOP_ON_FAILURE)

println " For each n every split or default invoice Amount which needs to pay is display in delivery Final summary screen"
...
Mobile.callTestCase(findTestCase('Phase2.1/InvoiceSplitInvoiceReplacement/SplitInvoiceReplacementAndProductBuying/Screenshot'),
	[('testCaseName') : 'TC_164'], FailureHandling.STOP_ON_FAILURE)
