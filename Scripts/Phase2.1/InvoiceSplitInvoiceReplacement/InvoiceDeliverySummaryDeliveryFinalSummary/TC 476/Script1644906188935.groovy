...
def remaining = Double.parseDouble(CashAmount) - Double.parseDouble(PesitioAmt)
def PesitoCommission = Double.parseDouble(findTestData('Phase2.1/TY_05/Collection').getValue(5, 14))
def PartialComm = PesitoCommission-1
def PartialCommission= findTestData('Phase2.1/TY_05/Collection').getValue(4, 58)
def Amount = remaining.toString()-PartialCommission.toString()

Mobile.verifyElementText(findTestObject('Object Repository/Phase2/BIInvoiceSummaryScreen/BICollectionScreen01/Amount_EditText'), findTestData('Phase2.1/TY_05/Collection').getValue(4, 66), FailureHandling.STOP_ON_FAILURE)
...
//Mobile.setText(findTestObject('Object Repository/Phase2/BIInvoiceSummaryScreen/BICollectionScreen01/Amount_EditText'), findTestData('Phase2.1/TY_05/Collection').getValue(3, 58), 0)
...
Mobile.callTestCase(findTestCase('Reusable Cases/Mobile/Phase2.1/ElectronicTransferMode'), [('ReferenceNum') : findTestData('Phase2.1/TY_05/Collection').getValue(3, 63)],FailureHandling.STOP_ON_FAILURE)

Mobile.verifyNotMatch(TransAmount, PesitioAmt, false, FailureHandling.STOP_ON_FAILURE)

//def ChequeAmt= Amount.toString()-TransAmount.toString()
...
Mobile.setText(findTestObject('Object Repository/Phase2/BIInvoiceSummaryScreen/BICollectionScreen01/Amount_EditText'), PartialCommission.toString() , 0)
...
println('Actualresult=' + Actualtoastmessage1)

def Expectedtoastmessage1 = findTestData('Phase2.1/TY_05/Collection').getValue(3, 54)
//def Expectedtoastmessage = findTestData('Phase2.1/TY_05/Collection').getValue(3, 56)

println('Expectedresult=' + Expectedtoastmessage1)

Mobile.verifyEqual(Actualtoastmessage1, Expectedtoastmessage1, FailureHandling.STOP_ON_FAILURE)

//Mobile.verifyNotEqual(Actualtoastmessage1, Expectedtoastmessage1, FailureHandling.STOP_ON_FAILURE)
...
def TransMode =  Double.parseDouble(TransAmount)+Double.parseDouble(PartialCommission)

Mobile.setText(findTestObject('Object Repository/Phase2/BIInvoiceSummaryScreen/BICollectionScreen01/Amount_EditText'), TransMode.toString() , 0)
...
//Mobile.verifyNotEqual(Actualtoastmessage, Expectedtoastmessage, FailureHandling.STOP_ON_FAILURE)

println(Actualtoastmessage)


println("Pesito Commission '3' should pay in Efectivo Toast message should not Enter in cash mode and remaining amount Electronic Transfer and Cheque mode.")
...
Mobile.callTestCase(findTestCase('Phase2.1/InvoiceSplitInvoiceReplacement/InvoiceDeliverySummaryDeliveryFinalSummary/Screenshot'),
	[('testCaseName') : 'TC_476'], FailureHandling.STOP_ON_FAILURE)
