...
def remInvAmt=Double.parseDouble(invAmt)-Double.parseDouble(creditAmt)

def partialAmt=remInvAmt/2

DecimalFormat df=new DecimalFormat('0.00')

partialAmt=df.format(partialAmt)

def remAmt=remInvAmt- Double.parseDouble(partialAmt)

remAmt=df.format(remAmt)
...
Mobile.tap(findTestObject('Object Repository/Phase2/BINumberKeyboardCollection01/3_NumButton'), 0)
...
Mobile.callTestCase(findTestCase('Phase2.1/InvoiceSplitInvoiceReplacement/InvoiceDeliverySummaryDeliveryFinalSummary/Screenshot'), 
    ['testCaseName':'TC_429_CollectionScreen'], FailureHandling.STOP_ON_FAILURE)
...
Expected_Toast_Message=Expected_Toast_Message+remAmt

println(Expected_Toast_Message)

Mobile.verifyMatch(toastMsg, Expected_Toast_Message, false, FailureHandling.STOP_ON_FAILURE)
