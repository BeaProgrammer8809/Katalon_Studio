...
def EffAmtBfr=Mobile.getText(findTestObject('Object Repository/Phase2/BICollectionScreen01/BIEfectivoScreen01/Amount_EditText'), 0)

for (int i = 0; i < EffAmtBfr.length(); i++) {
	Mobile.tap(findTestObject('Phase2/BINumberKeyboardCollection01/BackSpace_NumButton'), 0)
}

Mobile.tap(findTestObject('Object Repository/Phase2/BINumberKeyboardCollection01/2_NumButton'), 0)

def EffAmtAfter=Mobile.getAttribute(findTestObject('Object Repository/Phase2/BICollectionScreen01/BIEfectivoScreen01/Amount_EditText'),'text', 0)

Mobile.verifyNotEqual(EffAmtBfr, EffAmtAfter, FailureHandling.STOP_ON_FAILURE)

println('Pesito commission displaying under effectivo mode is editable')
...
Mobile.callTestCase(findTestCase('Phase2.1/InvoiceSplitInvoiceReplacement/InvoiceDeliverySummaryDeliveryFinalSummary/Screenshot'), 
    ['testCaseName':'TC_427_CollectionScreen'], FailureHandling.STOP_ON_FAILURE)
