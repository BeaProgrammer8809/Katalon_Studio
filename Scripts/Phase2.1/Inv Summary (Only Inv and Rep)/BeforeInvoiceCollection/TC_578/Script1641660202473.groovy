...
def invAmt4 = Mobile.getText(findTestObject('Phase2/BIInvoiceSummaryScreen/BICollectionScreen01/Amount_EditText'), 0)

println('Invoice Amount in Collection screen is ' + invAmt4)

println(invAmt4.length())

for (int i = 0; i < invAmt4.length(); i++) {
...
Mobile.setText(findTestObject('Phase2/BICollectionScreen01/BITransferenciasElectronicasScreen01/ReferenceNum_EditText'), 
    findTestData('Phase2.1/TestData/Test_Data').getValue(6, 19), 0)

Mobile.tap(findTestObject('Phase2/BIInvoiceSummaryScreen/BICollectionScreen01/Submit_Button'), 0)

String Actual = Mobile.getText(findTestObject('Phase2/BIInvoiceSummaryScreen/Summary_ScreenTitle'), 0)

Mobile.verifyEqual(Actual, findTestData('Phase2.1/TestData/Test_Data').getValue(2, 19), FailureHandling.STOP_ON_FAILURE)

Mobile.verifyEqual(ComissionAmount, findTestData('Phase2.1/TestData/Test_Data').getValue(3, 19), FailureHandling.STOP_ON_FAILURE)

Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Only Inv and Rep)/BeforeInvoiceCollection/Screenshot'), [('testCaseName') : 'TC_578'], FailureHandling.STOP_ON_FAILURE)
