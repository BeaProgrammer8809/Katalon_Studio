...
def value = Mobile.getText(findTestObject('Phase2/BICollectionScreen01/BIEfectivoScreen01/Amount_EditText'), 0)
...
def PesitoGrant = Mobile.getText(findTestObject('Phase2/BIInvoiceSummaryScreen/BICollectionScreen01/Amount_EditText'), 0)

double PesitoGrantval = Double.parseDouble(PesitoGrant)

Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Only Inv and Rep)/Collection/Screenshot'), [('testCaseName') : 'TC_499'], 
    FailureHandling.STOP_ON_FAILURE)

Mobile.comment('Pesito granted amount should come based on amount collected in pesito grant mode')

Mobile.verifyMatch(PesitoGrant, InvoiceAmt, false, FailureHandling.STOP_ON_FAILURE)

Mobile.verifyMatch(PesitoGrant, value, false, FailureHandling.STOP_ON_FAILURE)
