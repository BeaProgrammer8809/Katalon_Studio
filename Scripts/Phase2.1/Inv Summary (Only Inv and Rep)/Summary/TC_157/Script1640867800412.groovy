...
def ItemdAndCategisccount = Mobile.getText(findTestObject('Phase2/BIInvoiceSummaryScreen/BIAmountSplitUpPopup01/CompDisc_Value'), 0)
println(ItemdAndCategisccount)

def split=ItemdAndCategisccount.split(' ')

compDiscText1 =split[0]
compDiscText2 =split[1]
ItemandCategoryDiscount=compDiscText1+compDiscText2

println(ItemandCategoryDiscount)
...
def ItemmandCategdiscountss=Double.parseDouble(Discountedprice)-Double.parseDouble(SKUGross)
println(ItemmandCategdiscountss)

def ItemmandCategdiscountss1=ItemmandCategdiscountss
//def ItemmandCategdiscountss1="-"+ItemmandCategdiscountss
println(ItemmandCategdiscountss1)

Mobile.verifyEqual(ItemandCategoryDiscount, ItemmandCategdiscountss1, FailureHandling.STOP_ON_FAILURE)
println('Since ItemandCategoryDiscount and  ItemmandCategdiscountss1 are same ItemAndCategorydiscountss are applied')
...
println("Since SKUGross and Discountedprice are not same , discount applied")

Mobile.verifyNotEqual(TotalValue, Discountedprice, FailureHandling.STOP_ON_FAILURE)
println("Since price and TotalValue are same  tax  applied")

Mobile.verifyNotEqual(TotalOrderValue, ordervalue, FailureHandling.STOP_ON_FAILURE)
println("Since TotalOrderValue and  ordervalue are not equal isccount is  applied")
...
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Only Inv and Rep)/Summary/Screenshot'), [('testCaseName') : 'TC_157'], FailureHandling.STOP_ON_FAILURE)
