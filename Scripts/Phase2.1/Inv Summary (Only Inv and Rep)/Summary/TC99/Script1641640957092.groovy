...
for(int i=0; i<2; i++)
{
	if(i==0){
		...
		GlobalVariable.ProductName=findTestData('Phase2/Common_Data/CommonData_03').getValue( 6, 24)
	}
...
expectedTotalDiscount=df.format(expectedTotalDiscount)
...
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Only Inv and Rep)/Summary/Screenshot'), [('testCaseName') : 'TC99_InvoiceSummary'], 
...
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Only Inv and Rep)/Summary/Screenshot'), [('testCaseName') : 'TC99_PrintPreviewScreen'],
	FailureHandling.STOP_ON_FAILURE)
