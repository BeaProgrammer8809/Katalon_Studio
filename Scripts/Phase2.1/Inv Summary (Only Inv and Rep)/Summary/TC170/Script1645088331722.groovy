...
//def productName = findTestData('Phase2.1/Common_Data/CommonData').getValue(6, 14)
GlobalVariable.ProductName = findTestData('Phase2.1/Common_Data/CommonData').getValue(6, 17)
...
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Only Inv and Rep)/Summary/Screenshot'), [('testCaseName') : 'TC170'], FailureHandling.STOP_ON_FAILURE)
...
def ActualTotalPrice = Integer.parseInt(Pieces_Value) * Double.parseDouble(U_Price_VAlue)

println('Total price against sku1 is ' + ActualTotalPrice)

//Discount= Order Value - ActualTotalPrice
double Discount = Double.parseDouble(Price_Value) - ActualTotalPrice
...
def Expected_Order_Value = Price_Value - Discount

Mobile.verifyEqual(Price_Value, Expected_Order_Value, FailureHandling.STOP_ON_FAILURE)
