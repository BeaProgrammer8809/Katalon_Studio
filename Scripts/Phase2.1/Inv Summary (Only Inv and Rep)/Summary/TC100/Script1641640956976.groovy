...
println(totalPriceAgainstInvoicedSKU1)

Mobile.verifyEqual(DiscountedPriceSKU1, totalPriceAgainstInvoicedSKU1, FailureHandling.STOP_ON_FAILURE)
...
println(totalPriceAgainstInvoicedSKU2)

Mobile.verifyEqual(DiscountedPriceSKU2, totalPriceAgainstInvoicedSKU2, FailureHandling.STOP_ON_FAILURE)

println('Replacemenmt qty is not included in total price calculation')

double totalPriceAgainstInvoicedSKU = (totalPriceAgainstInvoicedSKU1) + (totalPriceAgainstInvoicedSKU2)

println(totalPriceAgainstInvoicedSKU)

double tax = Double.parseDouble(actualOrderValueForSKU) - totalPriceAgainstInvoicedSKU

println('IVA Tax applied for these products is ' + tax)
...
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Only Inv and Rep)/Summary/Screenshot'), [('testCaseName') : 'TC100_PrintPreviewScreen'],
	FailureHandling.STOP_ON_FAILURE)
