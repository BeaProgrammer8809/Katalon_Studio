...
GlobalVariable.ProductName = findTestData('Phase2.1/Common_Data/CommonData').getValue(6, 40)
...
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Only Inv and Rep)/Summary/Screenshot'), [('testCaseName') : 'TC171'], FailureHandling.STOP_ON_FAILURE)
...
//def totalSum = GrossAmt - TotalDisc

def actualTaxPercentage = findTestData('Phase2.1/CommonData/CommonData').getValue('IVA%', 1)

def expTaxPercentage1 = CustomKeywords.'com.ty.keywords.MobileKeywords.taxPercentage'(tax,totalSum)


expTaxPercentage = expTaxPercentage1.toString()

Mobile.verifyMatch(actualTaxPercentage, expTaxPercentage, false, FailureHandling.OPTIONAL)

println('Tax  IVA detail for sku')

def actualTaxPercentage1 = findTestData('Phase2.1/CommonData/CommonData').getValue('IEPS%', 1)

def expTaxPercentage2 = CustomKeywords.'com.ty.keywords.MobileKeywords.taxPercentage'(tax,totalSum)


expTaxPercentage3 = expTaxPercentage2.toString()

Mobile.verifyMatch(actualTaxPercentage1, expTaxPercentage3, false, FailureHandling.OPTIONAL)

println('Tax  IEPS detail for sku')
