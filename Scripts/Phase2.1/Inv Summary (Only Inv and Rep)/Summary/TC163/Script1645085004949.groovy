...
def ComposiDiscount = Mobile.getText(findTestObject('Phase2/BIInvoiceSummaryScreen/BIAmountSplitUpPopup01/CompDisc_Value'),
	0)
println(ComposiDiscount)


def split=ComposiDiscount.split(' ')

//compDiscText1 =split[0]
ComposiDiscounts =split[1]
//ItemDiscoun=compDiscText1+compDiscText2

println(ComposiDiscounts)

def ComposiDiscountss=Double.parseDouble(ComposiDiscounts)
...
def compDiscSymbol = ComposiDiscount.substring(0, 1)
...
def ItemDisc9 = CustomKeywords.'com.ty.keywords.MobileKeywords.item5_Percent'(SKUGross)
...
Mobile.verifyMatch(ComposiDiscountss.toString(), ItemDisc9.toString(), false, FailureHandling.STOP_ON_FAILURE)
println "Item disc applied for SKU"
