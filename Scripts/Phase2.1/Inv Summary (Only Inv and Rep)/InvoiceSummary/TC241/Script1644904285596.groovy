...
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Only Inv and Rep)/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC_241_AmtSplitup'], 
    FailureHandling.STOP_ON_FAILURE)
...
//splitDisc=compDiscValueText.split(" ")
...
ExpectedSign = findTestData('Phase2.1/TY_13/TY_13').getValue(10, 30)
...
Mobile.verifyMatch(TotalDiscount.toString(), CompDiscAbsValue.toString(), false)
...
double TotalCalc = OrderValue - CompDiscAbsValue

println(TotalCalc + 'TOTAL CALCULATED')

'Since there is no Tax and Discount applied to the SKUs the TotalAmount and TotalValue should be equal to the SKU gross amount  '
Mobile.verifyEqual(SKUGrossAmount, ValueDisplayed)

Mobile.verifyEqual(SKUGrossAmount, TotalOrderValue)
...
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Only Inv and Rep)/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC_241_Preticket'], 
    FailureHandling.STOP_ON_FAILURE)
