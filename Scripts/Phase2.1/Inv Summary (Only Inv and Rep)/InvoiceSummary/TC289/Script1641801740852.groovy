...
def ExpectedEntity = findTestData('Phase2.1/TY_01/Test_Data').getValue(4, 9)

Mobile.verifyMatch(legalEntity, ExpectedEntity, false, FailureHandling.STOP_ON_FAILURE)

Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Only Inv and Rep)/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC_289_FolioPopup'],
...
Mobile.verifyEqual(PrintPreviewscreen, findTestData('Phase2.1/TY_01/Test_Data').getValue(5, 9), FailureHandling.STOP_ON_FAILURE)

println('Invoice is be generated while doing only invoice for folio number enabled store with non-multi org route')

Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Only Inv and Rep)/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC_289'],
	FailureHandling.STOP_ON_FAILURE)
