...
/*Get the text of Total invoice quantity */
def InvoiceQuantity = Mobile.getText(findTestObject('Object Repository/Phase2/BIOrderAndInvoiceScreen01/Total_Qty_Value'),
	0)
...
Mobile.callTestCase(findTestCase('Reusable Cases/Mobile/Phase2.1/OnlyProduct_Buying_Quantity'), [('testData1') : findTestData(
			'Phase2.1/Common_Data/CommonData').getValue('ProductName', 44)], FailureHandling.STOP_ON_FAILURE)
...
def TotalQuantityinSummary = Mobile.getText(findTestObject('Object Repository/Phase2/BIInvoiceSummaryScreen/Total_Qty_Value'),
	0)
...
def Tax = Double.parseDouble(findTestData('Phase2.1/TY_05/Testdata').getValue('IEPSANDIVA', 1))

def ValueCalculation = skutotal + ((skutotal * Tax) / 100)

def CalculatedValue = Math.round(ValueCalculation)

int FinalCalculatedvalue = ((CalculatedValue) as int)

def ActualValue = Math.round(doublevalue)

int FinalActualValue = ((ActualValue) as int)

Mobile.verifyEqual(FinalActualValue, FinalCalculatedvalue, FailureHandling.STOP_ON_FAILURE)

/*Verification done to check Total quantity equal to Invoice quantity*/
Mobile.verifyEqual(InvoiceQuantity, TotalQuantityinSummary, FailureHandling.STOP_ON_FAILURE)

Mobile.callTestCase(findTestCase('Phase2.1/Inv summary (Only Replacement and pb)/Invoice summary/Screenshot'), [('testCaseName') : 'TC109'],
