...
GlobalVariable.ProductName = findTestData('Phase2.1/Common_Data/CommonData').getValue(16, 11)
...
def ActualFinal=ActualRes.substring(0, ActualRes.length()-7)
println "$ActualFinal"

	
	   
	  def ActualFinaluuid=ActualRes.split("$ActualFinal", 0)
	  def ActualFinalUUID=ActualFinaluuid[1]
	  println "$ActualFinalUUID"
...
def ActualFinalUUID1=ActualRes1.substring(0, ActualRes.length()-7)
println "$ActualFinalUUID1"
...
Mobile.verifyMatch(OnsiteInvoiceStatusReplacement.toString(),findTestData('Phase2.1/TY_05/Collection').getValue(6, 69), false,FailureHandling.STOP_ON_FAILURE)


Boolean InvoiceStatusPB=InvoiceSheet.contains(ActualFinalUUID1)

Mobile.verifyMatch(InvoiceStatusPB.toString(),findTestData('Phase2.1/TY_05/Collection').getValue(6, 69), false,FailureHandling.STOP_ON_FAILURE)

Boolean OnsiteInvoiceStatusProductBuying=InvoiceSheet.contains(Flag)

Mobile.verifyMatch(OnsiteInvoiceStatusProductBuying.toString(),findTestData('Phase2.1/TY_05/Collection').getValue(6, 69), false,FailureHandling.STOP_ON_FAILURE)


println("uuid message is matching with expected  ")

Mobile.callTestCase(findTestCase('Phase2.1/InvSummaryOnlyReplacementAndPB/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC_358'], FailureHandling.STOP_ON_FAILURE)
