...
Mobile.callTestCase(findTestCase('Phase2.1/InvSummaryOnlyReplacementAndPB/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC332_Print_Preview_Screen'], FailureHandling.STOP_ON_FAILURE)

def Expected_Onsite_Status = findTestData('Phase2.1/TY_02/Test_Data_Phase2').getValue(5, 218)
...
Mobile.verifyMatch(Status.toString(), findTestData('Phase2.1/TY_02/Test_Data_Phase2').getValue(7, 218), false, FailureHandling.STOP_ON_FAILURE)

//Verification to check the UUID number length
Mobile.verifyMatch(UUIDlength.toString(), findTestData('Phase2.1/TY_02/Test_Data_Phase2').getValue(6, 218), false, FailureHandling.STOP_ON_FAILURE)
