...
Mobile.callTestCase(findTestCase('Reusable Cases/Mobile/Phase2.1/ProductBuying_Qty'), [('testData1') : findTestData('Phase2.1/CommonData/CommonData').getValue(
            'Onsite Products', 32)], FailureHandling.STOP_ON_FAILURE)
...
def result = ActualPrinttext.split('PRODUCT BUYING')

def Expected_Print_Text = findTestData('Phase2.1/TestData/Test_Data').getValue(2, 86)

println(Expected_Print_Text)

def actualText1 = result[1]

boolean status = actualText1.contains(Expected_Print_Text)

Mobile.verifyMatch(status.toString(), findTestData('Phase2.1/TestData/Test_Data').getValue(6, 59), false, FailureHandling.STOP_ON_FAILURE)

Mobile.verifyEqual(printPrev, findTestData('Phase2.1/TestData/Test_Data').getValue(3, 63), FailureHandling.STOP_ON_FAILURE)

Mobile.callTestCase(findTestCase('Phase2.1/InvSummaryOnlyReplacementAndPB/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC_368'], 
