...
Mobile.callTestCase(findTestCase('Reusable Cases/Mobile/Phase2.1/Replacement_Quantity'), [('testData1') : findTestData('Phase2.1/Common_Data/CommonData').getValue(
            'Onsite Products', 13)], FailureHandling.STOP_ON_FAILURE)
...
Mobile.callTestCase(findTestCase('Phase2.1/InvSummaryOnlyReplacementAndPB/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC313_UUIDPopup'], 
...
Mobile.verifyMatch(UUIDlength.toString(), findTestData('Phase2.1/TY_01/TestData3').getValue(3, 26), false, FailureHandling.STOP_ON_FAILURE)
...
def expectedMessage = findTestData('Phase2.1/TY_01/TestData3').getValue(4, 26)

boolean MessageResult = actualMessage.contains(expectedMessage)

def verifytheToastMessage = MessageResult.toString()

Mobile.verifyMatch(verifytheToastMessage, findTestData('Phase2.1/TY_01/TestData3').getValue(5, 26), false, FailureHandling.STOP_ON_FAILURE)

Mobile.callTestCase(findTestCase('Phase2.1/InvSummaryOnlyReplacementAndPB/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC313'], 
    FailureHandling.STOP_ON_FAILURE)
