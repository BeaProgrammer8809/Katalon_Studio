...
double expectedTotal = Double.parseDouble(expectedtotal)
println(expectedTotal + "--Total displayed in Order and Invoice screen")
...
def compDiscTxt = Mobile.getText(findTestObject('Phase2/BIInvoiceSummaryScreen/BIAmountSplitUpPopup01/CompDisc_Value'), 
    0)

Mobile.tap(findTestObject('Phase2/BIInvoiceSummaryScreen/BIAmountSplitUpPopup01/Close_Button'), 0, FailureHandling.STOP_ON_FAILURE)

println(compDiscTxt + '--Actual Comp Disc Value')

def CompdiscLen = compDiscTxt.length()

double compDiscAmt = Double.parseDouble(compDiscTxt.substring(2, CompdiscLen))


println(compDiscAmt + '--Comp Disc Amount in Double Format')

def skuTotalValue = (expectedTotal - compDiscAmt)
println(skuTotalValue + '--calculated SKU Total ')
String SkuTotalValue  = String.format("%.2f", skuTotalValue)
println(SkuTotalValue + "--Converted Sku result")
...
Mobile.verifyElementText(findTestObject('Phase2/BIInvoiceSummaryScreen/BISummaryProductDetails/Price_Value_Indexing'), SkuTotalValue, 
...
Mobile.verifyElementText(findTestObject('Phase2/BIInvoiceSummaryScreen/Value_Value'), SkuTotalValue, FailureHandling.STOP_ON_FAILURE)
...
Mobile.callTestCase(findTestCase('Phase2.1/InvSummaryOnlyReplacementAndPB/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC06'],
	FailureHandling.STOP_ON_FAILURE)
