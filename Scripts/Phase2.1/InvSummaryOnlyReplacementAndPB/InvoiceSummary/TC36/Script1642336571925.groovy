...
Mobile.callTestCase(findTestCase('Reusable Cases/Mobile/Phase2.1/Trade_Coverage_Generic'), [('retailerName') : findTestData(
            'Phase2.1/Common_Data/CommonData').getValue('RetailerCash', 13)], FailureHandling.STOP_ON_FAILURE)
...
Mobile.verifyElementText(findTestObject('Phase2/BIInvoiceSummaryScreen/Summary_ScreenTitle'), findTestData('Phase2.1/TY_06/Inv summary (Only Replacement and PB)/Invoice summary').getValue(
...
Mobile.callTestCase(findTestCase('Phase2.1/InvSummaryOnlyReplacementAndPB/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC36'], 
...
Mobile.verifyMatch(SkuTotalPrice.round().toString(), findTestData('Phase2.1/TY_06/Inv summary (Only Replacement and PB)/Invoice summary').getValue(
        4, 4), false, FailureHandling.STOP_ON_FAILURE)

'Tax calculation'
def IEPSTaxPercentageText = findTestData('Phase2.1/TY_06/Inv summary (Only Replacement and PB)/Invoice summary').getValue(
    5, 1)

double TotalpriceAfterTax = SkuTotalPrice + ((SkuTotalPrice * Double.parseDouble(IEPSTaxPercentageText)) / 100)

Mobile.comment('Total price should not consider the tax amount')

Mobile.verifyEqual(SkuTotalPrice, TotalpriceAfterTax, FailureHandling.STOP_ON_FAILURE)
