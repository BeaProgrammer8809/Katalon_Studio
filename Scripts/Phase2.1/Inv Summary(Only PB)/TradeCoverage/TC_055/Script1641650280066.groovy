...
def piecesUpdate = Mobile.getText(findTestObject('Phase2/BIUnload/NonSalable/NoSalable_Stock_Popup/Pieces_Edit_Text_Indexing'),
	0)

Mobile.tap(findTestObject('Phase2/BIUnload/NonSalable/NoSalable_Stock_Popup/Done_Button'), 0)

Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary(Only PB)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_055'],
	FailureHandling.STOP_ON_FAILURE)

Mobile.comment('Screen should redirect to Unload screen')
...
Mobile.comment('entered quantities in the popup are getting displayed into Unload screen')

Mobile.verifyElementText(findTestObject('Phase2/BIUnload/NonSalable/Total_Piece_Indexing'), piecesUpdate, FailureHandling.STOP_ON_FAILURE)

def pieceAfterUpdate = Mobile.getText(findTestObject('Phase2/BIUnload/NonSalable/Total_Piece_Indexing'), 0)

Mobile.verifyMatch(pieceAfterUpdate, piecesUpdate, false, FailureHandling.STOP_ON_FAILURE)

Mobile.comment('values should updated against the product So values before update and after update was different')

Mobile.verifyNotMatch(pieceAfterUpdate, PiecesBeforeUpdate, false, FailureHandling.STOP_ON_FAILURE)
