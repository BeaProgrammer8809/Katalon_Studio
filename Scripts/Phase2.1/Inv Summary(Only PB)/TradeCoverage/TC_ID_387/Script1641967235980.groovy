...
def Expected_Inv_Amt=Inv_Amt1[0]
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary(Only PB)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_ID_387_Expected_Inv_Amt'], FailureHandling.STOP_ON_FAILURE)
...
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary(Only PB)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_ID_387_Expected_Total_Amt_Present'], FailureHandling.STOP_ON_FAILURE)

Mobile.tap(findTestObject('Phase2/BINumberKeyboardCollection01/BackSpace_NumButton'), 0)
Mobile.tap(findTestObject('Phase2/BINumberKeyboardCollection01/BackSpace_NumButton'), 0)
Mobile.tap(findTestObject('Object Repository/Phase2/BINumberKeyboardCollection01/9_NumButton'), 0)
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary (Only Inv and Rep)/InvoiceSummary/Screenshot'), [('testCaseName') : 'TC_ID_386_Toast_Message'], FailureHandling.STOP_ON_FAILURE)

AppiumDriver driver = MobileDriverFactory.getDriver()
def Actual_Toast_Message = driver.findElementByXPath('//android.widget.Toast[1]').getText()
def Expected_Toast_Message = findTestData('Phase2.1/TY_02/Test_Data_Phase2').getValue(8, 41)

//Verification to check that Amount exceeds the balance and please check message is displayed
...
def Total_Amt1 = Mobile.getText(findTestObject('Object Repository/Phase2/BICollectionScreen01/BIEfectivoScreen01/TotalPaid_TextView'),0)
def amount2 = Total_Amt1.split(' ')
def Expected_Total_Amt = amount2[3]
println Expected_Total_Amt
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary(Only PB)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_ID_387_Expected_Total_Amt'], FailureHandling.STOP_ON_FAILURE)
...
println('Amount exceeds the balance and please check message is displayed and it is not allowing to enter amount greater than Invoice amount in Total paid field')
