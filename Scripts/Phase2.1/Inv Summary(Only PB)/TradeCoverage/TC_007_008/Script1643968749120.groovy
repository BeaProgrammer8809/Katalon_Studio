...
def ActualSKU = Mobile.getText(findTestObject('Phase2/BIProductBuyingScreen01/Scheme_Buy_SKU_Text_View'), 0)

println(ActualSKU)

def SKUQty = Mobile.getText(findTestObject('Phase2/BIProductBuyingScreen01/Total_Pieces_Qty'), 0)

println(SKUQty)

double skuQty = Double.parseDouble(SKUQty)
...
double SalesReturns = Double.parseDouble(Sales_Return)

println(SalesReturns)

DecimalFormat df = new DecimalFormat('0.00')

ActualSales_Return = df.format(SalesReturns)

Mobile.verifyEqual(skuQty, SalesReturns, FailureHandling.STOP_ON_FAILURE)

Mobile.verifyMatch(ActualSKU, ExpectedSKU, false, FailureHandling.STOP_ON_FAILURE)

println('SKU quantity is same as sales return in summary screen')

Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary(Only PB)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_008'], FailureHandling.STOP_ON_FAILURE)
