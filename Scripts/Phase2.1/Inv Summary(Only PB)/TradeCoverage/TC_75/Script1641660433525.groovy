...
Mobile.setText(findTestObject('Phase2/BIInvoiceSummaryScreen/BIEnterTheFolioNoPopup01/Only_ProductBuying_InvQty/FolioNo._EditText_Indexing'), 
    findTestData('Phase2.1/TestData/Test_Data').getValue(2, 39), 0)
...
def Undescore = Actual.substring(4, 5)

def SpaceChar = Actual.substring(9, 10)

println(SpaceChar)
println(Undescore)

def Expected = findTestData('Phase2.1/TestData/Test_Data').getValue(2, 39)

println(Expected)

Mobile.verifyEqual(SpaceChar, findTestData('Phase2.1/TestData/Test_Data').getValue(5, 39), FailureHandling.STOP_ON_FAILURE)


Mobile.verifyEqual(Undescore, findTestData('Phase2.1/TestData/Test_Data').getValue(4, 39), FailureHandling.STOP_ON_FAILURE)
...
Mobile.callTestCase(findTestCase('Phase2.1/Inv Summary(Only PB)/TradeCoverage/Screenshot'), [('testCaseName') : 'TC_75'], FailureHandling.STOP_ON_FAILURE)
