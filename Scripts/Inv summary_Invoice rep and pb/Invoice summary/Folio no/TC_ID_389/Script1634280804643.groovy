...
Mobile.callTestCase(findTestCase('Inv summary_Invoice rep and pb/Invoice summary/Screenshot'), [('testCaseName') : 'TC_ID_389'], 
...
Mobile.callTestCase(findTestCase('Inv summary_Invoice rep and pb/Invoice summary/Screenshot'), [('testCaseName') : 'TC_ID_389_01'], 
    FailureHandling.STOP_ON_FAILURE)
...
if (folioNo.contains('FOLIO NO.:')) {
    ...
    if (folioNo.contains(txtFolioNo)) {
        ...
        if (txtFolioNo.contains('-')) {
            println('special character - should present in the Print Preview screen.')
        } else {
            FailureHandling.STOP_ON_FAILURE
        }
    } else {
