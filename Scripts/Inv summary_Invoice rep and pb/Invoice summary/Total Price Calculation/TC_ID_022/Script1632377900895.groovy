...
if (Mobile.verifyMatch(txtProductTotalAmt, calculatedTotalPrice, false, FailureHandling.OPTIONAL)) {
    ...
    if (Mobile.verifyMatch(txtProductTotalAmt, calculatedTotalAmt, false, FailureHandling.OPTIONAL)) {
        ...
        if (Mobile.verifyMatch(txtProductTotalAmt, updatedCalculatedTotAmt, false, FailureHandling.OPTIONAL)) {
            Mobile.verifyMatch(txtProductTotalAmt, updatedCalculatedTotAmt, false, FailureHandling.STOP_ON_FAILURE)
        } else {
            float calculatedTotalAmt1 = Float.parseFloat(GlobalVariable.totalPrice)

            totAmt = String.format('%.2f', calculatedTotalAmt1)

            println(totAmt)

            Mobile.verifyMatch(txtProductTotalAmt, totAmt, false, FailureHandling.STOP_ON_FAILURE)

            GlobalVariable.totalPrice = totAmt
...
println('Before Update: ' + calculatedTotalAmt)
...
Mobile.callTestCase(findTestCase('Inv summary_Invoice rep and pb/Invoice summary/Screenshot'), [('testCaseName') : 'DataUploaded'], 
    FailureHandling.STOP_ON_FAILURE)
