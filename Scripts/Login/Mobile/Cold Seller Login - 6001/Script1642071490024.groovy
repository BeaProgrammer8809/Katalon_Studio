...
GlobalVariable.userName = findTestData('Login Credentials/Mobile/Login Info').getValue(1, 26)
println(GlobalVariable.userName)
GlobalVariable.password = findTestData('Login Credentials/Mobile/Login Info').getValue(2, 26)
...
if (Mobile.verifyElementVisible(findTestObject('Login/Mobile/Activation Screen/AK_Activation Key'), 15, FailureHandling.OPTIONAL)) {
   ...
   // Mobile.hideKeyboard()
	...
	Mobile.delay(5)
	if (Mobile.verifyElementVisible(findTestObject('Login/Mobile/Activation Screen/Choose the seller'), 10, FailureHandling.OPTIONAL)) {
		Mobile.tap(findTestObject('Login/Mobile/Activation Screen/Employee Name'), 0)
	}
	
	//Mobile.delay(60, FailureHandling.STOP_ON_FAILURE)
	
	Mobile.waitForElementPresent(findTestObject('Phase2/BIAccessThisDevicesLocationPopup01/While_Using_The_App_Button'), 60,
		FailureHandling.OPTIONAL)
	if (Mobile.verifyElementVisible(findTestObject('Phase2/BIAccessThisDevicesLocationPopup01/While_Using_The_App_Button'), 0, FailureHandling.OPTIONAL)) {
