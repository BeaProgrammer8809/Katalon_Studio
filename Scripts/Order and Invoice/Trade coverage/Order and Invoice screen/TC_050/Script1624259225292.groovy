...
txtOrder = Mobile.getText(findTestObject('Phase 1.1/Resource3/Order and Invoice/Trade Coverage/Store Activities Screeen/Order And Invoice Screen/Search/Special Filter/SF_Ordered'), 
    0)

Mobile.verifyMatch('Ordered', txtOrder, false, FailureHandling.STOP_ON_FAILURE)
...
if (Mobile.verifyElementVisible(findTestObject('Phase 1.1/Resource3/Product Buying/Product Buying'), 0, FailureHandling.STOP_ON_FAILURE)) {
    println('Application should allow to navigate with product buying screen on clicking next button with SIH filter is applied')

    Mobile.callTestCase(findTestCase('Order and Invoice/Trade coverage/Order and Invoice screen/Screenshot'), [('testCaseName') : 'TC_053'], 
        FailureHandling.STOP_ON_FAILURE)
} else {
    FailureHandling.STOP_ON_FAILURE
}
