...
Mobile.setText(findTestObject('Phase 1.1/Resource1/Van Seller/Product Buying/Search input field'), findTestData('Phase 1/Anen/Inv summary_invoice and pb/Sku List').getValue(
        'sku_only_tax_IEPS_and_Item_level_discounts', 1), 0)
...
Mobile.callTestCase(findTestCase('Inv summary_invoice and pb/Trade Coverage/Inv summary/Screenshot'), [('testCaseName') : 'TC_ID_122_170_218'], 
...
String a = Comp_Value.charAt(0)

println(a)

if (a.equalsIgnoreCase('.')) {
    Comp_Value = '0'.concat(Comp_Value)
}
...
Mobile.callTestCase(findTestCase('Inv summary_invoice and pb/Trade Coverage/Inv summary/Screenshot'), [('testCaseName') : 'TC_ID_028_072'], 
    FailureHandling.STOP_ON_FAILURE)
...
Mobile.callTestCase(findTestCase('Inv summary_invoice and pb/Trade Coverage/Inv summary/Screenshot'), [('testCaseName') : 'TC_ID_028_072_122(1)'], 
    FailureHandling.STOP_ON_FAILURE)
...
'select * from AppData_INT_Sales_Invoice_Header  SIH_No=\'MX21739_212001246004\' order by 1 desc'
...
split_total = (total_order - discountamt)

String splittotal = String.format('%.2f', split_total)

String compDisc = String.format('%.2f', discountamt)
...
Mobile.verifyMatch(splittotal_Value, splittotal, false)

Mobile.verifyMatch(Order_Value, totalorder, false)

Mobile.verifyMatch(Comp_Value, compDisc, false)
